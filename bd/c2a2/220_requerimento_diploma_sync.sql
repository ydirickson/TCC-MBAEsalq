-- Sincroniza requerimento de diploma via trigger no schema diplomas (cenario c2a2)
...
CREATE OR REPLACE PROCEDURE public.sync_requerimento_diploma_c2a2(
  p_vinculo_id BIGINT,
  p_pessoa_id BIGINT,
...
CREATE OR REPLACE FUNCTION public.fn_sync_requerimento_diploma_c2a2()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $$
BEGIN
  IF NEW.tipo_vinculo = 'ALUNO'
    ...
    IF TG_OP = 'INSERT'
      ...
      CALL public.sync_requerimento_diploma_c2a2(
        NEW.id,
        NEW.pessoa_id,
...
FOR EACH ROW
EXECUTE FUNCTION public.fn_sync_requerimento_diploma_c2a2();
