...
-- Sincroniza dados compartilhados de pessoa da pos_graduacao para os demais schemas (cenario c2a1)

CREATE OR REPLACE FUNCTION fn_sync_pessoa_pos_graduacao_targets()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $$
BEGIN
  ...
  INSERT INTO graduacao.pessoa (id, nome, data_nascimento, nome_social, criado_em, replicado_em)
  VALUES (NEW.id, NEW.nome, NEW.data_nascimento, NEW.nome_social, NEW.criado_em, clock_timestamp())
  ON CONFLICT (id) DO UPDATE SET
    nome = EXCLUDED.nome,
    data_nascimento = EXCLUDED.data_nascimento,
...
CREATE OR REPLACE FUNCTION fn_sync_documento_pos_graduacao_targets()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $$
BEGIN
  ...
  INSERT INTO graduacao.documento_identificacao (id, pessoa_id, tipo, numero)
  VALUES (NEW.id, NEW.pessoa_id, NEW.tipo, NEW.numero)
  ON CONFLICT (id) DO UPDATE SET
    pessoa_id = EXCLUDED.pessoa_id,
    tipo = EXCLUDED.tipo,
...
CREATE OR REPLACE FUNCTION fn_sync_contato_pos_graduacao_targets()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $$
BEGIN
  ...
  INSERT INTO graduacao.contato (id, pessoa_id, email, telefone)
  VALUES (NEW.id, NEW.pessoa_id, NEW.email, NEW.telefone)
  ON CONFLICT (id) DO UPDATE SET
    pessoa_id = EXCLUDED.pessoa_id,
    email = EXCLUDED.email,
...
CREATE OR REPLACE FUNCTION fn_sync_endereco_pos_graduacao_targets()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $$
BEGIN
  ...
  INSERT INTO graduacao.endereco (id, pessoa_id, logradouro, cidade, uf, cep)
  VALUES (NEW.id, NEW.pessoa_id, NEW.logradouro, NEW.cidade, NEW.uf, NEW.cep)
  ON CONFLICT (id) DO UPDATE SET
    pessoa_id = EXCLUDED.pessoa_id,
    logradouro = EXCLUDED.logradouro,
...
DROP TRIGGER IF EXISTS trg_sync_pessoa_pos_graduacao_targets ON pos_graduacao.pessoa;
CREATE TRIGGER trg_sync_pessoa_pos_graduacao_targets
AFTER INSERT OR UPDATE ON pos_graduacao.pessoa
FOR EACH ROW
EXECUTE FUNCTION fn_sync_pessoa_pos_graduacao_targets();

DROP TRIGGER IF EXISTS trg_sync_documento_pos_graduacao_targets ON pos_graduacao.documento_identificacao;
CREATE TRIGGER trg_sync_documento_pos_graduacao_targets
AFTER INSERT OR UPDATE ON pos_graduacao.documento_identificacao
FOR EACH ROW
EXECUTE FUNCTION fn_sync_documento_pos_graduacao_targets();

DROP TRIGGER IF EXISTS trg_sync_contato_pos_graduacao_targets ON pos_graduacao.contato;
CREATE TRIGGER trg_sync_contato_pos_graduacao_targets
AFTER INSERT OR UPDATE ON pos_graduacao.contato
FOR EACH ROW
EXECUTE FUNCTION fn_sync_contato_pos_graduacao_targets();

DROP TRIGGER IF EXISTS trg_sync_endereco_pos_graduacao_targets ON pos_graduacao.endereco;
CREATE TRIGGER trg_sync_endereco_pos_graduacao_targets
AFTER INSERT OR UPDATE ON pos_graduacao.endereco
FOR EACH ROW
EXECUTE FUNCTION fn_sync_endereco_pos_graduacao_targets();
