-- Sincroniza documento assinavel e status de emissao via trigger cross-schema (cenario c2a1)
...
CREATE OR REPLACE FUNCTION public.fn_sync_documento_assinavel_c2a1()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $$
DECLARE
...
DROP TRIGGER IF EXISTS trg_sync_documento_assinavel_c2a1 ON diplomas.documento_diploma;
CREATE TRIGGER trg_sync_documento_assinavel_c2a1
AFTER INSERT OR UPDATE ON diplomas.documento_diploma
FOR EACH ROW
EXECUTE FUNCTION public.fn_sync_documento_assinavel_c2a1();

-- Atualiza diplomas.status_emissao quando solicitacao de assinatura e concluida ou rejeitada
CREATE OR REPLACE FUNCTION public.fn_sync_status_emissao_c2a1()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $$
DECLARE
...
DROP TRIGGER IF EXISTS trg_sync_status_emissao_c2a1 ON assinatura.solicitacao_assinatura;
CREATE TRIGGER trg_sync_status_emissao_c2a1
AFTER INSERT OR UPDATE ON assinatura.solicitacao_assinatura
FOR EACH ROW
EXECUTE FUNCTION public.fn_sync_status_emissao_c2a1();
