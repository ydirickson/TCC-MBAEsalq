-- Sincroniza requerimento de diploma via trigger no schema diplomas (cenario c2a1)
...
BEGIN
  IF NOT EXISTS (
    ...
    ALTER TABLE diplomas.requerimento_diploma
      ADD CONSTRAINT uq_requerimento_diploma_vinculo
      UNIQUE (vinculo_id);
  END IF;
END $$;

CREATE OR REPLACE PROCEDURE public.sync_requerimento_diploma_c2a1(
  p_vinculo_id BIGINT,
  p_pessoa_id BIGINT,
...
BEGIN
  ...
  INTO v_pessoa_nome,
       ...
       v_curso_tipo,
       v_data_conclusao
  FROM diplomas.pessoa p
  JOIN diplomas.vinculo_academico v ON v.id = p_vinculo_id
  WHERE p.id = p_pessoa_id;

  v_data_conclusao := COALESCE(v_data_conclusao, p_data_solicitacao);

  INSERT INTO diplomas.requerimento_diploma (pessoa_id, vinculo_id, data_solicitacao)
  VALUES (p_pessoa_id, p_vinculo_id, p_data_solicitacao)
  ON CONFLICT ON CONSTRAINT uq_requerimento_diploma_vinculo
  DO UPDATE SET
    pessoa_id = EXCLUDED.pessoa_id,
    data_solicitacao = EXCLUDED.data_solicitacao
  RETURNING id INTO v_requerimento_id;

  INSERT INTO diplomas.base_emissao_diploma (
    requerimento_id,
    pessoa_id,
    pessoa_nome,
    pessoa_nome_social,
...
CREATE OR REPLACE FUNCTION public.fn_sync_requerimento_diploma_c2a1()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $$
BEGIN
  IF NEW.tipo_vinculo = 'ALUNO'
    ...
    IF TG_OP = 'INSERT'
      ...
      CALL public.sync_requerimento_diploma_c2a1(
        NEW.id,
        NEW.pessoa_id,
...
DROP TRIGGER IF EXISTS trg_sync_requerimento_diploma ON diplomas.vinculo_academico;
CREATE TRIGGER trg_sync_requerimento_diploma
AFTER INSERT OR UPDATE ON diplomas.vinculo_academico
FOR EACH ROW
EXECUTE FUNCTION public.fn_sync_requerimento_diploma_c2a1();
